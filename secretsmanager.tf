...
locals {
  rds_credentials = {
    ...
    password             = try(var.settings.managed_password, false) || local.snapshot_identifier == null ? null : random_password.randompass[0].result
  ...
  secret_name        = local.db_name != null ? format("%s/%s/%s/%s/master-rds-credentials", local.secret_store_path, var.settings.engine_type, module.this.db_instance_identifier, local.db_name) : ""
  secret_description = local.db_name != null ? format("RDS Aurora Master credentials - %s - %s - %s - %s", local.master_username, var.settings.engine_type, module.this.db_instance_name, local.db_name) : ""
}
